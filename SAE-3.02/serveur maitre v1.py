        ...
        try:
            ...
            if message_split[0] == '/file': #envoi de fichier par un client.
                ...
                try: #initialisation de l'envoi au serveur esclave pour l'execution du fichier.
                    ...
                    print(f"[INFO] Echec de l'envoi : {e}.")
                    print("[INFO] en attente de la sortie.")
            ...
            if message[:4] == '/out':
                ...
                try:
                    ...
                    print("[ERREUR] Une erreur est survenue, interruption.")
                    continue
...
def AcceptServeur():
    ...
    while not fin.is_set():
        try:
            ...
            if x[9] == 'None':
                x[9] = None
            liste_serveur.append([conn, [addr[0], addr[1]], [x[1], bool(x[2]), x[3]], [x[4], bool(x[5]), x[6]], [x[7], bool(x[8]), x[9]]]) #stockage des informations dans liste_serveur
            for x in liste_serveur:
                if x[0] == conn:
