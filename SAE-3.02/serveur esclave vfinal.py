import socket, sys, subprocess, threading, psutil, time, os, sys, random, platform
...
def ecoute():
    ...
    /refus = message envoyé en cas de refus de connexion entrante
    ...
    while not fin.is_set():
        try:
            ...
            if message_split[0] == "/file": #envoi de fichier par le serveur maître.
                ...
                err = '/err' + '/////' + message_split[3] + '/////' + str(message_split[4]) #message à renvoyer en cas d'erreur
                ...
                try:
                    ...
                    print("[ERREUR] Une erreur s'est produite lors de la réception d'un fichier.")
                    try:
                        client.send(err.encode('utf-8'))
                    except:
                        print("[ERREUR] Connexion avec le serveur maître perdue. Arrêt.")
                        fin.set()
                    continue
                ...
                if extension[1] == "cc" or extension[1] == "cpp":
                    try:
                        ...
                        print(f"[INFO] exécutable {extension[0]} créé.")
                        print(f'[INFO] Execution de : {extension[0]} .')
                        if platform.system() == "Windows": #séparation des tâches à effectuer en fonction du système détecté
                            exe = extension[0] + '.exe'
            ...
            if message == "/stop":
                ...
                fin.set()
            if message == "/refus":
                print(f"[ERREUR] Serveur complet.")
                fin.set()
            if message == "/arret":
